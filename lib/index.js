...
exports.getAlbumFans = function (albumUrl, cb) {
  req(albumUrl, function (error, html) {
    if (error) {
      cb(error, null);
    } else {
      var products = htmlParser.parseAlbumFans(html, albumUrl);
      cb(null, products);
    }
  });
};

exports.getFanInfo = function (fanUrl, cb) {
  req(fanUrl, function (error, html) {
    if (error) {
      cb(error, null);
    } else {
      var fanInfo = htmlParser.parseFanInfo(html, fanUrl);
      cb(null, fanInfo);
    }
  });
};
