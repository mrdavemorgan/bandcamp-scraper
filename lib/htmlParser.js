...
// parse album fans results
exports.parseAlbumFans = function (html) {
  var $ = cheerio.load(html);
  var data = scrapeIt.scrapeHTML($, {
    results: {
      listItem: "div #collectors-data",
      data: {
        id: { attr: 'id' },
        blob: { attr: 'data-blob' }
      }
  ...
  return data.results.reduce(function (results, result) {
    var object = assignProps(result, {}, ['id', 'blob']);
      try {
        object = JSON.parse(object.blob);
        if (ajv.validate('album-fans', object)) {
          return object;
        } else {
          console.error('Error: ', ajv.errorsText(), object, JSON.stringify(ajv.errors));
        }
...
// parse fan info
exports.parseFanInfo = function (html, fanUrl) {
  var $ = cheerio.load(html);
  var data = scrapeIt.scrapeHTML($, {
    div: {
      selector: 'div #fan-bio-pic',
      data: {
        imageUrl: {
          selector: "img",
          attr: 'src'
  ...
  var object = parseFanRawInfo($.html());
  if(object){
    object.fanbiopic = data.div.imageUrl;
    var coll = parseFanCollection($.html());
    if(coll){
      object.collection = coll;
    }
  }
  // validate through JSON schema
  if (ajv.validate('fan-info', object)) {
    return object;
  } else { // TODO add a flag to log only when debugging
    console.error('Validation error on fan info: ', ajv.errorsText(), object);
    return null;
  }
};


// parse fan info hidden in the javascript
var fiRegex = /var FanData = ({(.|\n)+?});/m;
var parseFanRawInfo = function (html) {
  var match = fiRegex.exec(html);
  if (match) {
    return JSON.parse(match[1].replace(jsonifyKeyRegex, '"$1": $2$3'));
  }
  return null;
};

// parse collection info hidden in the javascript
var cdRegex = /var CollectionData = ({(.|\n)+?});/m;
var parseFanCollection = function (html) {
  var match = cdRegex.exec(html);
  if (match) {
    var collection = JSON.parse(match[1].replace(jsonifyKeyRegex, '"$1": $2$3'));
    var collectionItems = [ ];
    for (var key in collection.item_details) {
      collectionItems.push(collection.item_details[key]);
    }
